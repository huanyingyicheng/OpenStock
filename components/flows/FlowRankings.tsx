            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-950 text-gray-400">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right font-medium">{t('flows.table.mainInflow')}</th>
                  <th className="px-4 py-3 text-right font-medium">{t('flows.table.mainOutflow')}</th>
                  <th className="px-4 py-3 text-right font-medium">{t('flows.table.superLargeNet')}</th>
                  <th className="px-4 py-3 text-right font-medium">{t('flows.table.largeNet')}</th>
                  <th className="px-4 py-3 text-right font-medium">{t('flows.table.changePct')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-800 bg-gray-900/20">
                {items.map((row) => {
                  ...
                  const mainInflow = typeof row.mainInflow === 'number' ? row.mainInflow : Math.max(row.netInflow ?? 0, 0);
                  const mainOutflow = typeof row.mainOutflow === 'number' ? row.mainOutflow : Math.max(-(row.netInflow ?? 0), 0);
                  const mainInflowClass = mainInflow > 0 ? 'text-teal-400' : 'text-gray-300';
                  const mainOutflowClass = mainOutflow > 0 ? 'text-red-400' : 'text-gray-300';
                  const superLargeClass =
                    typeof row.superLargeNet === 'number'
                      ? row.superLargeNet > 0
                        ? 'text-teal-400'
                        : row.superLargeNet < 0
                          ? 'text-red-400'
